...
struct Bar(u8, #[derivative(Default(value = "42"))] u8);
...
#[derivative(Default(new = "true"))]
struct Baz(
    #[derivative(Default(value = "[1,2]"))] [u8; 2],
    #[derivative(Default(value = "[3;2]"))] [u8; 2],
    #[derivative(Default(value = "(4,5)"))] (u8, u8),
    #[derivative(Default(value = "B1(6,7)"))] B1,
    #[derivative(Default(value = "B2{a:8,b:9}"))] B2,
);
...
fn main() {
    ...
    assert_eq!(
        Baz::new(),
        Baz([1, 2], [3, 3], (4, 5), B1(6, 7), B2 { a: 8, b: 9 })
    );
